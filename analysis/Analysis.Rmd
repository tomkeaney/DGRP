...
We create a massive trait correlation matrix using the `cor()` function. We calculate spearman correlation coefficients ($rho$) to estimate $r_g$, as these are more robust to non-normality than the default pearson method.  

We also use the `corr.test` function from the `psych` package to get confidence intervals and p-values adjusted for multiple testing for each correlation.

Note that we perform ~260,000 bivariate correlations here, so the code takes a while to run.
