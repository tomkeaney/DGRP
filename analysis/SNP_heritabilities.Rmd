...
Here we load in the unscaled phenotype data we collated [here](Data_collation.html). Note that we use the trimmed dataset that only includes traits that have been measured across >79 lines. This dataset also does not include data from two studies: Jin _et al_ (2020) and Everett _et al_ (2020). 
...
We then fix the names of the DGRP lines to conform with `GCTA` and `Plink` i.e. line is required in front of the line number e.g. `line21`.

```{r}
traits <- read_csv("data/derived/selected.dgrp.phenos_unscaled.csv") 
...
if(rerun_greml){
  ...
  lines_measured <- 
    traits %>% 
    group_by(Trait) %>% 
    summarise(lines_measured = n())
  
  trait_info <- left_join(trait_info, lines_measured)
  
  SNP_heritability <- left_join(variance_components, trait_info) %>% 
...
} else SNP_heritability <- read_csv("data/derived/SNP_heritability_cleaned.csv")
