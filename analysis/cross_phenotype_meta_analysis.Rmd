...
n_independent_traits <- get_independent_traits() %>% length()
```
...
## Calculate cross-phenotype meta-analysis (CPMA) statistic for each variant
...
In the code below, we calculate the CPMA test statistic and p-value for each of the `r n_snps` variants in the LD-pruned subset.
