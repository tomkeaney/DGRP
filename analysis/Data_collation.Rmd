...
library(osfr) # For accessing data on the repository OSF
```

$~$

## Loading the line mean phenotype data from Google sheets

$~$

A high percentage of the papers about the DGRP provide estimates of the mean phenotype of multiple DGRP lines ("line mean data"). We worked collaboratively to conduct a non-exhaustive search of the literature for phenotypic trait measurements for DGRP lines, and collated all the line mean data using Google Sheets. The R code below downloads the data from Google Sheets, saves it as a .csv file, and loads it into R. 

```{r}
remake_data_from_googlesheets <- FALSE

if(remake_data_from_googlesheets){
  ...
  line_mean_googlesheet_info <- gs4_get(line_mean_url)$sheets %>% 
    select(name, index) 
  number_sheets <- nrow(line_mean_googlesheet_info)
  
  line_means_phenos <- lapply(
    1:number_sheets, 
    function(i) {
      print(paste("Reading Google Sheet number ", i, " of ", number_sheets, "...", sep = ""))
      x <- read_sheet(line_mean_url, sheet = i)
      Sys.sleep(3)
      return(x)
    })
  ...
  reference_table <- do.call(
    "rbind", 
    lapply(line_means_phenos, 
           function(x) tibble(Trait = names(x)[2], 
                              Reference = x$reference[1], 
                              Sex = x$Sex[1], 
                              `Trait guild` = x$Guild[1], 
                              `Trait description` = x$Explanation[1])))
  
  line_means_phenos <- lapply(
    line_means_phenos, 
    function(x) x[,1:2])[order(-sapply(line_means_phenos, nrow))]
  
  
  all.dgrp.phenos.1 <- lapply(
    line_means_phenos, 
    function(x) {
      mutate(x, Trait = names(x)[2])}) 
  
  new_colnames <- c("line", "trait_value", "Trait") 
  
  all.dgrp.phenos.2 <- lapply(all.dgrp.phenos.1, setNames, new_colnames)
  
  all.dgrp.phenos <- do.call("rbind", all.dgrp.phenos.2) %>% 
    arrange(Trait, line) %>% 
    as.data.frame() %>% 
    left_join(reference_table, by = "Trait") %>%
    filter(!is.na(line) & !is.na(trait_value)) %>% 
  ...
  all.dgrp.phenos %>% 
    select(line, Trait, Sex, trait_value) %>% 
    write_csv("data/input/all.line.mean.phenos.csv")
  
  all.dgrp.phenos %>% 
    select(Trait, `Trait guild`, `Trait description`, Reference) %>% 
    distinct() %>% 
    write_csv("data/input/all.line.mean.phenos_metadata.csv")
  
  
} else {
  # if(!file.exists("data/input/all.dgrp.phenos.csv")){
  #   osf_retrieve_node("https://osf.io/2h5pw/") %>% osf_ls_files() %>%
  #     filter(name == "all.dgrp.phenos.csv") %>% 
  #     osf_download(path = "data/input")
  # }
  all.line.mean.phenos <- read_csv("data/input/all.line.mean.phenos.csv") %>% 
