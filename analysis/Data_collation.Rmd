...
all.line.mean.phenos <- read_csv("data/input/all.line.mean.phenos.csv") %>% 
  ...
  mutate(Sex = replace(Sex, Trait == "change.methamphetamine.preferenceA.f", "Female")) # fix this error in the input data
```
...
We also provide and save a list of references that appear in the original database, which is used to ensure that the analyses in the paper can be faithfully reproduced if new datasets are added later.

```{r}
tibble(Reference = sort(unique(c(read_csv("data/input/all.line.mean.phenos_metadata.csv")$Reference,
                            read_csv("data/derived/dgrp_phenos_calculated_from_raw_data_meta_data.csv")$Reference)))) %>% 
