...
## Loading the line mean phenotype data

$~$

A high percentage of the papers that report DGRP data provide estimates of the mean phenotype of multiple DGRP lines ("line mean data"). We worked collaboratively to conduct a non-exhaustive search of the literature for phenotypic trait measurements for DGRP lines, and collated all the line mean data in two csv files; `all.line.mean.phenos.csv` which contains line mean values for each trait and `all.line.mean.phenos_metadata.csv` which contains study meta-data. The R code below combines these files.
...
Some studies only provide the raw data they collected when measuring DGRP phenotypes. We need line mean data to calculate genetic correlations between traits and run typical GWA analyses. To get line means, we fit Bayesian mixed effect models and use these to estimate mean trait values for each line. Each analysis is presented in a separate tab below.
...
The Bayesian models won't run unless you have `stan` and the `brms` package installed. To install `stan` follow these [instructions](https://github.com/stan-dev/rstan/wiki/RStan-Getting-Started). Otherwise, you can skip to the data cleaning section and load in the csv file `"data/input/all.dgrp.phenos_combined.csv"` as a tibble/dataframe.
...
# create a list of all traits to iterate our function over
...
-   Battlay et al (2016): lines 226, 847 and 917 - there is no obvious reason for mysterious lines. No obvious typos and the paper states they GWA for 178 lines, which matches the number reported in File S2, which is where we sourced the data.
...
-   Montgomery et al (2014): lines 424 and 766 - Errors appear to be mislabelled lines. Table S3 includes RAL lines 424 and 766. Line 766 is out of numeric order in the Table. Most likely to actually be line 776 or 786. No way to tell.
