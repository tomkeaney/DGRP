...
fitness_young_f_line_means <- Wong_Holman_line_means("fitness.early.f", "Mean absolute number of first instar larvae produced when 2-5 day old focal DGRP females were placed in a vial with competitor females and standard males. Focal females and their same-sex competitors were not replaced when they died, such that the fitness assays depend upon mortality as well as offspring production. Higher values indicate higher fitness.")

fitness_old_f_line_means <- Wong_Holman_line_means("fitness.late.f", "Mean absolute number of first instar larvae produced when 14-17 day old focal DGRP females were placed in a vial with competitor females and standard males. Focal females and their same-sex competitors were not replaced when they died, such that the fitness assays depend upon mortality as well as offspring production. Higher values indicate higher fitness.")

fitness_young_m_line_means <- Wong_Holman_line_means("fitness.early.m", "Mean proportion of first instar larvae produced when 2-5 day old focal DGRP males were placed in a vial with competitor males and standard females. Focal males and their same-sex competitors were not replaced when they died, such that the fitness assays depend upon mortality as well as offspring production. Higher values indicate higher fitness.")

fitness_old_m_line_means <- Wong_Holman_line_means("fitness.late.m", "Mean proportion of first instar larvae produced when 2-5 day old focal DGRP males were placed in a vial with competitor males and standard females. Focal males and their same-sex competitors were not replaced when they died, such that the fitness assays depend upon mortality as well as offspring production. Higher values indicate higher fitness.")
...
After checking Bloomington (<https://bdsc.indiana.edu/Home/Search>) and flybase (<http://flybase.org/reports/FBlc0000504.html>) we found the following anomalies.
...
-   Watanabe ad Riddle (2021): line 854 - not on flybase
...
-   Katzenberger et al (2015): line 521 - no obvious typo (data is ordered by increasing trait value). One-off issue.
...
This still leaves 219 lines, which considering most studies report the DGRP to consist of 205 lines, is unexpected.

However, the DGRP record on flybase (<http://flybase.org/reports/FBlc0000504.html>) indicates that a total of 219 lines have been included in the DGRP at some point since its creation. We have phenotype measurements for all 219 lines, and this can be explained by some lines being withdrawn from the Bloomington stock centre. Lines were withdrawn because they were very sick, sequence data was not available, or they were essentially duplicates (that is, they exhibit high genomic relatedness with another one of the lines) of other lines.

The lines that have been withdrawn but are included in our dataset are: 80, 272, 343, 378, 387, 393, 398, 476, 514, 554, 556, 591, 750, 771. We choose **not to remove** these lines from our dataset, as many of the studies we include in our dataset make use of these lines.
...
**If there are 0 rows in the problems tibble that we create in the below code chunk, it means we've fixed them all**
...
### Save and show the cleaned dataset
...
# how many traits did we start with: all.dgrp.phenos_combined %>% distinct(Trait)
#after cleaning: all.dgrp.phenos_combined_subset %>% distinct(Trait)

```


All up we have removed 18 traits, taking our total from 1232 to 1214

The full list of traits we document can be viewed here:
...
full_table <- left_join(lines_measured, table) %>% 
  select(-line) %>% 
  arrange(Trait)
...
my_data_table(full_table)
```


$~$

### Trim data for our analysis
...
Correlation estimates are imprecise for traits that have been measured across a small number of lines. Therefore, we further trim the dataset to only include traits that have been phenotyped across \>80 lines.
...
trait_test <- 
  selected.dgrp.phenos_unscaled %>% group_by(Trait) %>% 
  summarise(n_lines = length(line)) %>% 
  arrange(n_lines)

# Remove traits that have been measured in < 80 lines

selected.dgrp.phenos_unscaled <- 
  selected.dgrp.phenos_unscaled %>% 
  group_by(Trait) %>% 
  filter(length(line) > 79) %>% 
  ungroup() %>% 
  arrange(Reference)
...
```{r}
write_csv(selected.dgrp.phenos_unscaled, file = "data/derived/selected.dgrp.phenos_unscaled.csv")
```


This leaves us with 516 traits for analysis. They can be viewed here:
...
table <- selected.dgrp.phenos_unscaled %>% 
  distinct(Trait, .keep_all = TRUE) %>% 
  arrange(Trait) %>% 
  select(-c(trait_value))
  

trimmed_table <- left_join(lines_measured, table) %>% 
  select(-line) %>% 
  arrange(Trait)
    ...
    options = list(
      ...
      buttons =
        list('pageLength', 'colvis', 'csv', list(
          ...
          pageSize = 'A4',
          orientation = 'landscape',
          filename = 'Analysis_trait_table')),
      pageLength = 516
    )
  )
}

my_data_table(trimmed_table)
